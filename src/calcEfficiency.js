...
function filteringItem(raidItemLevel) {
    const rewards = {
        1730: { '운명의 파괴석 결정': 0, '운명의 수호석 결정': 0, '운명의 파편': 0, '위대한 운명의 돌파석': 0 },
        1620: { '운명의 파괴석': 0, '운명의 수호석': 0, '운명의 파편': 0, '운명의 돌파석': 0 },
...
function calcPrice(item, itemData, number) {
    ...
    let conversionRates = {
        '운명의 파괴석 결정': 100, '운명의 수호석 결정': 100, '운명의 파괴석': 100, '운명의 수호석': 100, 
        '정제된 파괴강석': 100, '정제된 수호강석': 100, '파괴강석': 100, '수호강석': 100,
        '파괴석 결정': 100, '수호석 결정': 100, '운명의 파편': 3000, '명예의 파편': 1500
    };
...
function calcEfficiency(raidList, itemData) {
    ...
    return raidList.map(raidName => {
        if (['카멘', '아브렐슈드'].includes(raidName)) return exceptionalHandling(raidName, itemData);
